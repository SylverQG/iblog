<template>
  <article class="max-w-4xl mx-auto px-4 py-8">
    <div v-if="!post" class="text-center py-12 text-gray-500">加载中...</div>

    <template v-else>
      <header class="mb-8">
        <h1 class="text-3xl font-bold mb-3 text-gray-900 dark:text-white">{{ post.title }}</h1>
        <div class="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-4">
          <time>{{ formatDate(post.createdAt) }}</time>
          <span v-if="post.updatedAt !== post.createdAt">更新于 {{ formatDate(post.updatedAt) }}</span>
        </div>
        <div class="flex flex-wrap gap-2">
          <TagBadge v-for="label in post.labels" :key="label" :tag="label" />
        </div>
      </header>

      <div class="prose" v-html="post.bodyHtml ?? ''" />
